...
const IconFlag = ({ filled, active }: { filled?: boolean; active?: boolean }) => (
  <svg viewBox="0 0 24 24" fill={filled || active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" width="16" height="16"
...
export function ProdutoForm({
  ...
  const FieldLabel = ({ label, field }: { label: string; field: 'categorias' | 'locais' | 'fornecedores' }) => (
    <div className="d-flex justify-content-between align-items-center mb-1">
      ...
      <button type="button" className="btn-manage-discreet" style={{ height: '24px', padding: '0 6px' }}
        onClick={() => { setManageField(field); setShowManageModal(true); }} title="Gerenciar histórico">
        <IconGear />
      </button>
    </div>
  );
  ...
  return (
    <form onSubmit={submit}>
      <div className="mb-3" style={{ fontSize: '13.5px', color: 'var(--text-2)' }}>
        Estoque atual: <strong style={{ color: 'var(--text-1)' }}>{produto.quantidade} {produto.unidade}</strong>
      ...
      <div className="text-end mt-4 d-flex justify-content-end gap-2">
        <button type="button" className="btn btn-secondary" onClick={onCancel}><IconX /> Cancelar</button>
        <button type="submit" className="btn btn-primary"><IconCheck /> Salvar Movimentação</button>
      </div>
    </form>
  );
}

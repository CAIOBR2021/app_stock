...
export default function App() {
  ...
  const handleGenerateDeliveryReport = () => {
    ...
    const telefone    = first.responsavelTelefone ? formatPhoneNumber(first.responsavelTelefone) : 'Não informado';
    ...
    doc.setFillColor(...AMBER); doc.roundedRect(ML, 5, 18, 18, 2, 2, 'F');
    ...
    doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(180, 195, 225);
    doc.text('Sistema de Controle de Estoque', ML + 22, 20);
    doc.setFontSize(8); doc.setTextColor(180, 195, 225);
    ...
    y += 3; doc.setFillColor(...AMBER); doc.rect(ML, y, 55, 1.5, 'F'); y += 7;
    doc.setFillColor(...LIGHT); doc.setDrawColor(...BORDER); doc.roundedRect(ML, y, CW, 16, 3, 3, 'FD');
    ...
    doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.setTextColor(...TEXT3);
    ...
    doc.setDrawColor(...BORDER); doc.line(ML + CW * 0.62, y + 2, ML + CW * 0.62, y + 14);
    doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.setTextColor(...TEXT3);
    ...
    doc.setFillColor(...AMBER); doc.rect(ML, y, 3, 6, 'F');
    doc.text('RELAÇÃO DE ENTREGAS', ML + 6, y + 5); y += 10;
    const colLocal    = 52;
    ...
    (doc as any).autoTable({
      ...
      body: tableBody,
      headStyles: { fillColor: NAVY, textColor: WHITE, fontStyle: 'bold', fontSize: 8, cellPadding: { top: 5, bottom: 5, left: 3, right: 3 }, halign: 'center', minCellHeight: 10 },
      bodyStyles: { fontSize: 8.5, textColor: TEXT1, cellPadding: { top: 5, bottom: 5, left: 4, right: 4 }, valign: 'middle' },
      columnStyles: {
      ...
      didDrawCell: (data: any) => {
        ...
        if (data.column.index === 0) {
          ...
          doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...TEXT1);
        ...
        if (data.column.index === 1) {
          ...
          doc.setDrawColor(...BORDER); doc.setLineWidth(0.3); doc.rect(bx, by, sz, sz);
        }
      },
    ...
    selected.forEach(d => { totaisPorLocal[d.localObra] = (totaisPorLocal[d.localObra] || 0) + 1; });
    ...
    if (spaceForResume >= 80 && locaisUnicos.length > 0) {
      ...
      doc.setFillColor(...NAVY); doc.roundedRect(ML, resumY, CW, 14, 2, 2, 'F');
      doc.setFontSize(7.5); doc.setFont('helvetica', 'bold'); doc.setTextColor(...AMBER);
      doc.text('RESUMO POR OBRA:', ML + 5, resumY + 9);
      let rx = ML + 46;
      locaisUnicos.slice(0, 5).forEach(local => {
        const count = totaisPorLocal[local];
        doc.setTextColor(180, 195, 225); doc.setFont('helvetica', 'normal');
        const label = `${local}: `; doc.text(label, rx, resumY + 9); rx += doc.getTextWidth(label);
        doc.setTextColor(...WHITE); doc.setFont('helvetica', 'bold');
        const val = `${count} item(ns)   `; doc.text(val, rx, resumY + 9); rx += doc.getTextWidth(val);
      });
    }
    ...
    const sigY = spaceLeft < spaceNeeded ? sigPageH - 50 : finalY + (spaceLeft > 80 ? 28 : 16);
    doc.setDrawColor(...BORDER); doc.setLineWidth(0.4);
    const sigLineW = CW * 0.36;
    doc.line(ML, sigY, ML + sigLineW, sigY);
    doc.line(pageW - MR - sigLineW, sigY, pageW - MR, sigY);
    doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(...TEXT3);
    ...
    for (let i = 1; i <= totalPages; i++) {
      doc.setPage(i);
      doc.setFillColor(...NAVY); doc.rect(0, pageH - 14, pageW, 14, 'F');
      doc.setFontSize(7.5); doc.setFont('helvetica', 'normal'); doc.setTextColor(180, 195, 225);
  ...
  const produtosAbaixoMinimo = useMemo(
    () => allProdutos.filter((p) => p.estoqueMinimo != null && p.quantidade <= p.estoqueMinimo),
    [allProdutos],
  );
  ...
  const locaisArmazenamento = useMemo(
    () => Array.from(new Set(allProdutos.map((p) => p.localArmazenamento || '').filter(Boolean))),
    [allProdutos],
  );
  ...
  const filteredDeliveries = useMemo(() => {
    ...
    if (rotaDateFilter)
      data = data.filter(
        (d) => new Date(d.dataHoraSolicitacao).toLocaleDateString('en-CA') === rotaDateFilter,
      );
    return data.sort(
      (a, b) => new Date(a.dataHoraSolicitacao).getTime() - new Date(b.dataHoraSolicitacao).getTime(),
    );
  ...
  const filteredProdutos = useMemo(() => {
    ...
    let result = allProdutos.filter((p) => {
      ...
      const searchableText = [p.nome, p.sku, p.categoria ?? '', p.descricao ?? '', p.localArmazenamento ?? '', p.fornecedor ?? '']
        .join(' ').toLowerCase();
  ...
  return (
    <div className="app-layout">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          {(
            ...
            <button key={id} className={`nav-item-clean ${view === id ? 'active' : ''}`} onClick={() => { setView(id); scrollTop(); }}>
              <Icon /> {label}
            </button>
          ))}
        </nav>
        <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 'auto' }}>
          <div style={{ fontSize: '10px', fontWeight: 500, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.8px', textTransform: 'uppercase' }}>Desenvolvido por</div>
          <div style={{ fontSize: '12px', fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.3px', marginTop: '2px' }}>Caio Vinícius de Carvalho Bezerra</div>
        </div>
      </aside>
      ...
      <div className="mobile-header d-lg-none d-flex justify-content-between align-items-center px-3">
        ...
        <div onClick={() => setShowLowStockModal(true)} style={{ cursor: 'pointer', position: 'relative', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div className="d-flex flex-column gap-3 mb-3">
                  <div className="d-flex align-items-center gap-2">
                    <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid var(--border)', borderRadius: '8px', overflow: 'hidden', background: '#fff', maxWidth: '220px' }}>
                      ...
                      <input type="date" className="form-control" style={{ border: 'none', borderRadius: 0, boxShadow: 'none', padding: '0 8px 0 0' }} value={rotaDateFilter} onChange={(e) => setRotaDateFilter(e.target.value)} />
                      {rotaDateFilter && (
                        <button style={{ background: 'none', border: 'none', padding: '0 10px', cursor: 'pointer', color: 'var(--text-3)', display: 'flex', alignItems: 'center' }} onClick={() => setRotaDateFilter('')} title="Limpar data"><IconX /></button>
                      )}
                    </div>
                  </div>
                  <div style={{ background: '#fff', padding: '12px 16px', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', flexWrap: 'wrap', gap: '12px', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div className="d-flex align-items-center gap-2 flex-wrap">
                      ...
                      <button className="btn btn-sm d-flex align-items-center gap-1" style={{ background: 'var(--success-light)', color: 'var(--success)', border: '1.5px solid var(--success)', borderRadius: '7px', height: '30px', padding: '0 10px', fontSize: '12.5px', fontWeight: 600 }} onClick={() => handleBulkStatusChange('Entregue')} disabled={selectedEntregaIds.length === 0}><IconCheck /> Entregue</button>
                      <button className="btn btn-sm d-flex align-items-center gap-1" style={{ background: 'var(--warning-light)', color: 'var(--primary-dark)', border: '1.5px solid var(--warning)', borderRadius: '7px', height: '30px', padding: '0 10px', fontSize: '12.5px', fontWeight: 600 }} onClick={() => handleBulkStatusChange('Pendente')} disabled={selectedEntregaIds.length === 0}><IconRefresh /> Pendente</button>
                    </div>
                    <div className="d-flex gap-2">
                      <button className="btn btn-ghost btn-sm d-flex align-items-center gap-1" disabled={selectedEntregaIds.length === 0} onClick={() => setShowReprogramModal(true)}><IconReprog /> Reprogramar</button>
                      <button className="btn btn-secondary btn-sm d-flex align-items-center gap-1" disabled={selectedEntregaIds.length === 0} onClick={handleGenerateDeliveryReport}><IconPDF /> PDF</button>
                    </div>
                  </div>
                </div>
      ...
      {showScroll && <button className="btn-scroll-top" onClick={scrollTop}><IconArrowUp /></button>}

      {/* ── MODALS ── */}

      {showLowStockModal && (
        <ModalComponent title="Alertas de Estoque" onClose={() => setShowLowStockModal(false)}>
          <p style={{ fontSize: '13px', color: 'var(--text-3)', marginBottom: '16px' }}>Produtos abaixo da quantidade mínima ({produtosAbaixoMinimo.length}):</p>
          {produtosAbaixoMinimo.length === 0 ? (
              ...
              {produtosAbaixoMinimo.map((p) => (
                <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ background: 'var(--danger-light)', color: 'var(--danger)', fontWeight: 700, fontSize: '13.5px', padding: '3px 10px', borderRadius: '999px', marginBottom: '2px' }}>{p.quantidade} {p.unidade}</div>
      ...
      {showReprogramModal && (
        <ModalComponent title="Reprogramar Entregas" onClose={() => setShowReprogramModal(false)}>
          <p style={{ fontSize: '13.5px', color: 'var(--text-2)' }}>Reprogramar <strong>{selectedEntregaIds.length}</strong> entrega(s).</p>
          <div className="mb-3">

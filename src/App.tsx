...
export default function App() {
  ...
  async function addProduto(p: Omit<Produto, 'id' | 'criadoEm' | 'atualizadoEm' | 'sku'>) {
    try {
      const res = await fetch(`${API_URL}/produtos`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p) });
      if (!res.ok) throw new Error();
      const novoProduto = await res.json();
      setAllProdutos(prev => [novoProduto, ...prev]);
    } catch { console.error('addProduto failed'); }
  }

  async function updateProduto(id: UUID, patch: Partial<Omit<Produto, 'id' | 'sku' | 'criadoEm'>>) {
    try {
      const res = await fetch(`${API_URL}/produtos/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) });
      if (!res.ok) throw new Error();
      const updated = await res.json();
      setAllProdutos(prev => prev.map(x => x.id === id ? updated : x));
    } catch { console.error('updateProduto failed'); }
  }
  ...
  async function deleteProduto(id: UUID) {
    ...
    } catch { console.error('deleteProduto failed'); }
  }
  ...
  async function togglePrioritario(id: UUID, currentState: boolean) {
    ...
    try {
      const res = await fetch(`${API_URL}/produtos/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prioritario: !currentState }) });
      if (!res.ok) throw new Error();
    } catch {
  ...
  async function addMov(m: Omit<Movimentacao, 'id' | 'criadoEm'>, custoEntrada?: number) {
    try {
      const res = await fetch(`${API_URL}/movimentacoes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...m, custoEntrada }) });
  ...
  const handleEntradaSaidaSubmit = async (dados: any) => {
    try {
      ...
      for (const item of dados.itens) {
        await addMov({ produtoId: item.produtoId, tipo: dados.tipo, quantidade: item.quantidade, motivo: motivoFinal, nomeObra: dados.nomeObra || undefined, ordemCompra: dados.ordemCompra || undefined, custoUnitarioHistorico: item.valorUnitario }, dados.tipo === 'entrada' ? item.valorUnitario : undefined);
      }
  ...
  async function updateMov(id: UUID, patch: { quantidade: number; motivo?: string }) {
    try {
      const res = await fetch(`${API_URL}/movimentacoes/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) });
      if (!res.ok) throw new Error();
    ...
    } catch { console.error('updateMov failed'); }
  }
  ...
  async function deleteMov(id: UUID) {
    try {
      ...
      const eRes = await fetch(`${API_URL}/entregas`);
      setEntregas((await eRes.json()).map(normalizeEntrega));
    } catch { console.error('deleteMov failed'); }
  }
  ...
  async function updateEntregaFull(id: string, data: any) {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/entregas/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
      if (!res.ok) { const e = await res.json(); throw new Error(e.error || 'Falha ao atualizar'); }
      const [eRes, pRes, mRes] = await Promise.all([fetch(`${API_URL}/entregas`), fetch(`${API_URL}/produtos?_limit=10000`), fetch(`${API_URL}/movimentacoes`)]);
      setEntregas((await eRes.json()).map(normalizeEntrega));
  ...
  async function addEntrega(data: any) {
    try {
      const res = await fetch(`${API_URL}/entregas`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
  ...
  const handleSaveDelivery = (data: any) => {
    ...
    if (produto) {
      ...
      if (editingEntrega && (editingEntrega.produtoId === produto.id || editingEntrega.itemNome === produto.nome)) estoque += Number(editingEntrega.itemQuantidade);
      if (Number(String(data.itemQuantidade).replace(',', '.')) > estoque) { setPendingDeliveryData(data); setShowStockLimitModal(true); return; }
    }
  ...
  async function updateEntregaStatus(id: string, status: string) {
    try {
      await fetch(`${API_URL}/entregas/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
  ...
  const confirmBulkStatusChange = async () => {
    try {
      setLoading(true);
      await Promise.all(selectedEntregaIds.map(id => fetch(`${API_URL}/entregas/${id}/status`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: bulkTargetStatus }) })));
      const eRes = await fetch(`${API_URL}/entregas`);
      setEntregas((await eRes.json()).map(normalizeEntrega));
      setSelectedEntregaIds([]);
      setShowBulkConfirmModal(false);
      toast.success(`Entregas marcadas como ${bulkTargetStatus}.`);
    } catch { toast.error('Erro ao atualizar status em massa.'); }
    finally { setLoading(false); }
  };

  const handleSelectEntrega = (id: string) =>
    setSelectedEntregaIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);

  // ── PDF ENTREGAS ─────────────────────────────────────────────────────────

  const handleGenerateDeliveryReport = () => {
    if (selectedEntregaIds.length === 0) { toast.error('Selecione ao menos uma entrega.'); return; }
    const selected = entregas.filter(d => selectedEntregaIds.includes(d.id) && !isDelivered(d.status))
                             .sort((a, b) => new Date(a.dataHoraSolicitacao).getTime() - new Date(b.dataHoraSolicitacao).getTime());
    if (selected.length === 0) { toast.error('Nenhum item pendente selecionado.'); return; }
    const first      = selected[0];
    const reportDate = new Date(first.dataHoraSolicitacao).toLocaleDateString('pt-BR');
    const responsavel = first.responsavelNome || 'Nao informado';
    const telefone    = first.responsavelTelefone ? formatPhoneNumber(first.responsavelTelefone) : 'Nao informado';
    ...
    doc.setTextColor(180,195,225); doc.text('Data:',pageW-MR-doc.getTextWidth('Data:  ')-doc.getTextWidth(reportDate),9);
    ...
    doc.setFont('helvetica','normal'); doc.setTextColor(...LGRAY); doc.text('|',cx,y); cx+=doc.getTextWidth('|   ');
    doc.setTextColor(...GRAY); doc.text('Telefone:',cx,y); cx+=doc.getTextWidth('Telefone:  ');
    doc.setFont('helvetica','bold'); doc.setTextColor(...DARK); doc.text(telefone,cx,y);
    y+=4; doc.setDrawColor(...BORDER); doc.setLineWidth(0.25); doc.line(ML,y,ML+cw,y); y+=8;
    ...
    (doc as any).autoTable({
      head:[['N.','OK','Hora','Local da Obra','Material','Qtd','Un','Armazem']],
      body:selected.map((d,i)=>[String(i+1).padStart(2,'0'),'',new Date(d.dataHoraSolicitacao).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}),d.localObra,d.itemNome||'-',String(d.itemQuantidade),d.itemUnidadeMedida||'-',d.localArmazenagem||'-']),
      startY:y,margin:{left:ML,right:MR},tableWidth:cw,
      columnStyles:{0:{cellWidth:colW[0],halign:'center',fontStyle:'bold'},1:{cellWidth:colW[1],halign:'center'},2:{cellWidth:colW[2],halign:'center'},3:{cellWidth:colW[3],halign:'left'},4:{cellWidth:colW[4],halign:'left',fontStyle:'bold'},5:{cellWidth:colW[5],halign:'center'},6:{cellWidth:colW[6],halign:'center'},7:{cellWidth:colW[7],halign:'center'}},
      headStyles:{fillColor:HEADER,textColor:WHITE,fontStyle:'bold',fontSize:8,cellPadding:{top:5,bottom:5,left:4,right:4},halign:'center'},
      bodyStyles:{fontSize:8.5,textColor:DARK,cellPadding:{top:6,bottom:6,left:4,right:4}},
      alternateRowStyles:{fillColor:LIGHT},
      styles:{lineColor:BORDER,lineWidth:0.2,valign:'middle'},
      didDrawCell:(data:any)=>{if(data.section==='body'&&data.column.index===1){const sz=3.8,bx=data.cell.x+(data.cell.width-sz)/2,by=data.cell.y+(data.cell.height-sz)/2;doc.setDrawColor(...LGRAY);doc.setLineWidth(0.3);doc.rect(bx,by,sz,sz);}},
    });
    const endY=(doc as any).lastAutoTable.finalY;
    const sigY=endY+22,lineLen=cw*0.36,lX=ML,rX=ML+cw-lineLen;
    doc.setDrawColor(...GRAY); doc.setLineWidth(0.4);
    doc.line(lX,sigY,lX+lineLen,sigY); doc.line(rX,sigY,rX+lineLen,sigY);
    doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.setTextColor(...GRAY);
    doc.text('Assinatura do Motorista',lX+lineLen/2,sigY+5,{align:'center'});
    doc.text('Assinatura do Solicitante',rX+lineLen/2,sigY+5,{align:'center'});
  ...
  const handleReprogramDeliveries = async () => {
    ...
    const valid = selectedEntregaIds.filter(id => { const e = entregas.find(e => e.id === id); return e && !isDelivered(e.status); });
    if (valid.length === 0) { toast.error("Apenas itens pendentes podem ser reprogramados."); setShowReprogramModal(false); return; }
    try {
      await Promise.all(valid.map(id => {
        const e = entregas.find(e => e.id === id);
        if (!e) return Promise.resolve();
        const time = e.dataHoraSolicitacao.split('T')[1] || '08:00:00';
        return fetch(`${API_URL}/entregas/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dataHoraSolicitacao: `${newDeliveryDate}T${time}` }) });
      }));
  ...
  const produtosAbaixoMinimo = useMemo(() =>
    allProdutos.filter(p => p.estoqueMinimo != null && p.quantidade <= p.estoqueMinimo), [allProdutos]);
  ...
  const filteredDeliveries = useMemo(() => {
    let data = entregas;
    if (rotaDateFilter) data = data.filter(d => new Date(d.dataHoraSolicitacao).toLocaleDateString('en-CA') === rotaDateFilter);
  ...
  const filteredProdutos = useMemo(() => {
    ...
    let result = allProdutos.filter(p => {
      ...
      return (q2 === '' || p.nome.toLowerCase().includes(q2) || p.sku.toLowerCase().includes(q2) || p.categoria?.toLowerCase().includes(q2))
    ...
    if (sortOrder) result = [...result].sort((a,b) => sortOrder === 'asc' ? a.nome.localeCompare(b.nome) : b.nome.localeCompare(a.nome));
    return result;
  ...
  const handleToggleSort = () => setSortOrder(cur => cur === null ? 'asc' : cur === 'asc' ? 'desc' : null);
      ...
      <div className="mobile-header d-lg-none d-flex justify-content-between align-items-center px-3">
        ...
        >
          ...
          {produtosAbaixoMinimo.length > 0 && (
            <span className="notif-count" style={{ top: '-2px', right: '-2px' }}>{produtosAbaixoMinimo.length}</span>
          )}
        </div>
      </div>
      ...
      <main className="main-content">
        ...
        <header className="page-header d-none d-lg-flex">
          ...
          >
            ...
            {produtosAbaixoMinimo.length > 0 && <span className="notif-count">{produtosAbaixoMinimo.length}</span>}
          </div>
        </header>
        ...
        {view === 'estoque' && (
          <div className="content-area animate-fade-in">
                ...
                <div className="d-flex align-items-center gap-3">
                  ...
                  {!loadingAll && (
                    <span style={{ fontSize: '13px', color: 'var(--text-3)' }}>
                      ...
                      {' '}de{' '}
                      <strong style={{ color: 'var(--text-1)' }}>{filteredProdutos.length}</strong> produtos
                    </span>
                  )}
                </div>
            ...
            <div className="mt-4 d-flex justify-content-center">
              {!loading && !loadingAll && (
                <Paginacao totalItems={filteredProdutos.length} itemsPerPage={ITEMS_PER_PAGE} currentPage={page} onPageChange={setPage} />
              )}
            </div>
                ...
                <div className="d-flex flex-column gap-3 mb-3">
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid var(--border)', borderRadius: '8px', overflow: 'hidden', background: '#fff', maxWidth: '220px' }}>
                      <span style={{ padding: '0 10px', color: 'var(--text-3)', display: 'flex', alignItems: 'center' }}><IconCalendar /></span>
                      <input
                        type="date"
                        className="form-control"
